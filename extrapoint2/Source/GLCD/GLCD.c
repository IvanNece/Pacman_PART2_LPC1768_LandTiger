...
void LCD_ClearWindow(int x, int y, int width, int height, uint16_t color) {
	int i,j;	
	for (i = 0; i < height; i++) {
        for (j = 0; j < width; j++) {
...
void GUI_Text_Narrow(uint16_t Xpos, uint16_t Ypos, uint8_t *str, uint16_t Color, uint16_t bkColor) {
    uint8_t TempChar;
    int X_offset = 0;
    ...
    do {
        TempChar = *str++;
        if (TempChar != 0) {
            // Disegna ogni carattere con meno spaziatura
            PutChar(Xpos + X_offset, Ypos, TempChar, Color, bkColor);
            X_offset += 6;  // Ridotto rispetto al normale valore 8
        }
    } while (TempChar != 0);
}
