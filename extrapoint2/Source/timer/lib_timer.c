...
Il file lib_timer.c fornisce le funzioni per gestire i timer hardware presenti nel microcontrollore LPC1768. 
Questi timer sono separati dal Repetitive Interrupt Timer (RIT) e sono progettati per attività come la temporizzazione di eventi,
la generazione di segnali PWM, o il controllo di timeout.
**--------------------------------------------------------------------------------------------------------
*********************************************************************************************************/
#include "LPC17xx.h"
#include "timer.h"
...
{
	//timer_num: Il numero del timer da avviare (0, 1, 2 o 3).
	//Imposta il bit 0 del registro TCR (Timer Control Register) a 1 per avviare il timer specificato.
  if ( timer_num == 0 )
  {
		LPC_TIM0->TCR = 1;
  }
...
{
	//Il bit 1 del registro TCR (Timer Control Register) viene impostato a 1 per resettare il timer, poi riportato a 0.
  uint32_t regVal;
...
//INIT TIMER *********************************************************************************************************************************
// Inizializza il timer con un intervallo specificato e abilita un registro di match.
/*
timer_num: Il numero del timer (0, 1, 2 o 3).
Prescaler: Valore del prescaler per ridurre la velocità del clock.
MatchReg: Quale registro di match configurare (0, 1, 2 o 3).
SRImatchReg: Configura il comportamento del registro di match (interrupt, reset, stop).
TimerInterval: Valore di confronto per il registro di match.
*/
uint32_t init_timer ( uint8_t timer_num, uint32_t Prescaler, uint8_t MatchReg, uint8_t SRImatchReg, uint32_t TimerInterval )
{
  if ( timer_num == 0 )
  {
		...
		//MR0: Valore del match register per il timer.
		//MCR: Configura cosa succede quando il valore del contatore coincide con MR0:
		//0x1: Genera un interrupt.
		//0x2: Resetta il contatore.
		//0x4: Ferma il timer
		if(MatchReg == 0)
		{
